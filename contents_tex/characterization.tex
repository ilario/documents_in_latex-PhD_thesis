...
\section{Conventions and General Remarks}
	...
	\subsection{Sign Convention and Parameters Definitions}
		...
		\begin{equation} \label{eq:photodiode}
			J = \jsc(\phi) - J_0\left(\exp(\frac{qV}{n_{id}k_BT})-1\right)
		\end{equation}
		...
		$$J = J_{ph}(\phi) - J_0\left(\exp(\frac{q(V+JR_s)}{n_{id}k_BT})-1\right)$$
...
\section{V\textsubscript{OC} and J\textsubscript{SC} Dependence on Light Intensity}
	...
	\subsection{\Glsentryshort{voc} \textit{versus} $\phi$ and the Ideality Factor $m$}
		...
		$$\jsc(\phi) = J_0\left(\exp(\frac{q\voc (\phi)}{n_{id}k_BT})-1\right)$$
		...
		$$\voc = \frac{n_{id}k_BT}{q}\cdot\ln\left(\frac{\jsc}{J_0} + 1\right)$$
		...
		$$\voc \approx u_1 + \frac{n_{id}k_BT}{q}\cdot\ln(\jsc)$$
		...
		\begin{equation}\label{eq:voc_vs_phi}
			\voc (\phi) \approx u_2 + \frac{n_{id}k_BT}{q}\cdot\ln(\phi)
		\end{equation}
...
\section{Charge Extraction (\glsentryshort{ce})}
	...
	\subsection{Interpretation of Charge Extraction}\label{interpretation_ce}
		...
		$$n_{CE} = n_{DOS} \exp(\frac{qV - E_g}{k_BT}) = n_{0,CE} \exp(\frac{qV}{k_BT})$$
		...
		$$n_{CE} = n_{0,CE} \exp(\frac{qV}{mk_BT})$$
		...
		\begin{equation}\label{eq:ce_full}
			n_{CE} = C_g \cdot V + u_1 \cdot \left[\exp(\frac{qV}{mk_BT}) - 1\right]
		\end{equation}
...
\section{Transient PhotoVoltage (TPV)}
	...
	\begin{dmath*}
		n(V_0 + \Delta V) \approx n(V_0) + \Delta V \cdot \left.\frac{\partial n}{\partial V}\right\rvert_{V=V_0} = n(V_0) + \Delta V \cdot \left(C_g + \frac{u_1q}{mk_BT}\exp(\frac{qV_0}{mk_BT})\right)
%		n(V_0 + \Delta V) \approx n(V_0) + \Delta V \cdot \eval{\dv{n}{V}}_{V=V_0} = n(V_0) + \Delta V \cdot \left(C_g + \frac{u_1q}{mk_BT}\exp\left(\frac{qV_0}{mk_BT}\right)\right)
	\end{dmath*}
...
%		\frac{\partial (n_0 + \Delta n)}{\partial t} = g - k_1(n_0 + \Delta n) - k_2(n_0 + \Delta n)^2 = (g - k_1 n_0 - k_2 n_0^2) - (k_1 \Delta n + 2 k_2 n_0 \Delta n) - (k_2 \Delta n ^2) \approx - (k_1 + 2 k_2 n_0 ) \Delta n
	...
	\begin{equation}\label{eq:tpv_monoexp}
		n (t) = n_0 + \Delta n_0 \cdot e^{-(k_1 + 2 k_2 n_0) t} = n_0 + \Delta n_0 \cdot e^{-t / \tau_{\mathrm{pfo}}}
	\end{equation}
	where $\tau_{\mathrm{pfo}} = (k_1 + 2 k_2 n_0)^{-1}$ is the pseudo first order life-time.
	For a single recombination with a generic recombination order $\Phi$ we can thus write:
	\begin{equation}\label{eq:tpv_tau_order}
		\tau_{\mathrm{pfo}} \propto (k_\Phi n_0^{\Phi-1})^{-1}
	\end{equation}
	...
	The main recombination mechanism in perovskite solar cells at open circuit conditions is the surface recombination (see \cpageref{intro_prv_recombination}) which, as described in \cpageref{intro_surface_recombination}, can be considered as a first order reaction with regards to the carriers in the perovskite.
	In \cref{eq:ce_full} the exponential part is related to the chemical capacitance of extra charges accumulating in the perovskite layer, excluding the charge accumulated in the contacts and electrodes.
	As the intrinsic 
	...
	This chemical charge is represented as the exponential part of \cref{eq:ce_full}, so we obtain:
	\begin{dmath}\label{eq:tpv_tau_vs_intensity}
		\tau_{\mathrm{pfo}} \propto (k_\Phi n_0^{\Phi-1})^{-1} \propto \left[\exp(\frac{q\voc}{mk_BT}) - 1\right]^{1-\Phi} \approx \exp(-\frac{(\Phi-1)q\voc}{mk_BT}) = \exp(-\frac{q\voc}{vk_BT})
	\end{dmath}
	where we used that $qV \gg k_BT$ for all non-dark cases.
	So the life-time typically decreases exponentially with the light bias with an ideality modifier of $v = m/(\Phi-1)$.
	This relation is in accordion with equivalent studies on \gls{osc} \cite{Shuttle2008d,Credgington2011}.
...
The pseudo first order life-times obtained AAAAAAAAAAAAAAAAAAAAAA
	In order to compare recombination constant via pseudo first order life-times, we have to make a comparison at the same charge concentrationAAAAAAAAAAAAAAAA.
...
From the \cref{eq:tpv_monoexp} it is clear that for the first case it is easy to obtain $k_1$ as it is equal to $\tau_{\mathrm{pfo}}^{-1}$.
But for the second case, to obtain $k_2$ is non-trivial as it is related to $\tau_{\mathrm{pfo}}$ via the charge density \cite{ORegan2007}.
For practical reasons in this thesis $\tau_{\mathrm{pfo}}$ is always considered, unless explicitly specified.
Comparisons of $\tau_{\mathrm{pfo}}$ have to be performed taking care of having similar charge densities in the devices under study if an information on underlying rate constants is sought \cite{ORegan2008}.
For this reason, rather than comparing \acr{tpv}, we usually compare life-times referenced to charge density, as in TPV-CE experimentAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAa
