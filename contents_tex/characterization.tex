...
%			The non-linear robust fitting (in brown) manages to reduce the weight of these points.
%			The studied device is a \gls{fto}\-/\dTiOtwo\-/\acr{famapbibr}\-/\spiro\-/Ag whose fabrication is not reported in this thesis.}\label{fig:tpv_robust}
%	\end{figure}
...
\section{Differential Capacitance (DC)}
	...
	\begin{itemize}
		...
		\item The sum of the two linear factors $\Delta V = \Delta V_1 + \Delta V_2$ in a bi-exponential fit as in \cref{eq:tpv_biexp}, which could work but one have to carefully set boundary values to the fitting parameters for avoiding a fast exponential matching just some noise.
