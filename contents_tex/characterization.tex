...
\section{Current-Voltage Sweeps}
	...
	The shunt and series resistances can be evaluated by the current \textit{versus} voltage derivative of a dark current-voltage sweep respectively at zero and at high-enough voltage.
...
%		The latter method has not been tested in this thesis as it would be affected by hysteresis.
		...
		A critical analysis of these methods and the proposal of a new "Transient Suns-\gls{voc}" method can be found in \authoryear{Calado2018b}.
		...
		While the ionic profile is flat through the perovskite layer, the electronic properties of the cell can be studied using the theory from \gls{osc} \textit{i.e.} without the influence of ionic accumulation.
...
\end{SCfigure}
	...
	It works \textit{via} the sudden extraction of the free charges present in an solar cell illuminated at open circuit conditions.
...
%	The transient current flowing through the short circuiting circuit can be measured and integrated to estimate the available charge.
	...
	Performing this experiment at various light intensities, we can obtain a charge density \textit{versus} light bias relationship.
...
The current flowing at any moment can be obtained from the measured voltage \textit{via} Ohm's law: $J=V/R$ where $R$ is \SI{50}{\ohm}.
...
The results are plotted as charge density \textit{versus} light bias.
...
%	\subsection{Interpretation of Charge Extraction}\label{interpretation_ce}
		...
		\begin{equation}
		n(V) = n_|DOS| \exp(\frac{qV - E_|g|}{k_|B|T}) = n_|i| \exp(\frac{qV}{k_|B|T})
		\end{equation}
		where $n_|DOS|$ is the density of states which can be occupied by free charges, $E_|g|$ is the material band gap, and $n_|i|=n_|DOS|\exp[-E_|g|/(k_|B|T)]$ prefactor is the intrinsic carrier concentration from a Boltzmann distribution of a 2 levels system. %just a pre-factor with no direct physical interpretation.
		As the \acr{ce} can just extract the excess charge $n_|CE|$, what we can measure is:
		\begin{equation}
		n_|CE|(V) = n(V)-n(0) = n_|i| \cdot \left[\exp(\frac{qV}{k_|B|T})-1\right]
		\end{equation}
		...
		\begin{equation}\label{eq:ce_osc}
			n_|CE| = n_|i| \cdot \left[\exp(\frac{qV}{mk_|B|T})-1\right]
		\end{equation}
...
\end{figure}
		...
		\begin{equation}\label{eq:ce_full}
			n_|CE| = C_|g| \cdot V + n_|i| \cdot \left[\exp(\frac{qV}{mk_|B|T}) - 1\right]
		\end{equation}
		...
		A voltage dependent geometric capacitance can be obtained from dark \acr{ce} measurements applying reverse voltage biases \cite{Kiermasch2018} or \textit{via} impedance spectroscopy in dark with a constant voltage bias \cite{Brus2016,Pockett2015}.
		...
		\makebox[\textwidth][c]{
			\parbox{1.1\textwidth}{
				...
				\mycaption[Simulation of a complete CE experiment: charge \textit{versus} light bias with or without mobile ions.]{
					...
					The solid lines represents the linear plus exponential fit crossing (0,0) obtained with the \cref{eq:ce_full} $n_|CE| = C_|g| V + n_|i| \{\exp[q V / (mk_|B|T)] - 1\}$ where $C_|g|$, $n_|i|$, and $m$ are free fitting parameters whose meaning is described in the text.}\label{fig:ce_full_dd}
			}
		}
	\end{figure}
...
%	While a complete device is kept open circuit under constant illumination, a small extra illumination is added \textit{via} a short laser pulse.
	This technique allows us to perturb and observe the a solar cell device at open circuit under illumination.
	We perturb the device photo-generating an additional small amount of charge and we observe the perturbed open circuit voltage dynamics.
...
%	From the \gls{voc} \textit{versus} illumination relation for photodiodes reported in \cref{eq:voc_vs_phi} follows that the \gls{voc}, at this new higher illumination, increases (this is not always the case, as for non-stabilized perovskite solar cells \cite{Calado2016}).
	...
	From this kinetic profile, a charge life-time can be obtained and plotted against the light bias due to the constant background illumination, as in the example of \cref{fig:tpv-monoexp}.
...
\end{SCfigure}
	...
	The fitted small perturbation life-time is plotted \textit{versus} the light bias.
...
This does not affect the parameter extraction from \acr{tpv} as just the high illumination intensity points are considered, as seen in \cpageref{tpv_tau_vs_intensity}, in order to study the device close to its expected working conditions.
	...
	\begin{dmath}
		n(V_0 + \Delta V) \approx n(V_0) + \Delta V \cdot \left.\frac{\partial n}{\partial V}\right\rvert_{V=V_0} = n(V_0) + \Delta V \cdot \left[C_|g| + \frac{n_|i| q}{mk_|B|T}\exp(\frac{qV_0}{mk_|B|T})\right]
%		n(V_0 + \Delta V) \approx n(V_0) + \Delta V \cdot \eval{\dv{n}{V}}_{V=V_0} = n(V_0) + \Delta V \cdot \left(C_|g| + \frac{n_|i| q}{mk_|B|T}\exp\left(\frac{qV_0}{mk_|B|T}\right)\right)
	\end{dmath}
...
%		\frac{\partial (n_0 + \Delta n)}{\partial t} = g - k_1(n_0 + \Delta n) - k_2(n_0 + \Delta n)^2 = (g - k_1 n_0 - k_2 n_0^2) - (k_1 \Delta n + 2 k_2 n_0 \Delta n) - (k_2 \Delta n ^2) \approx - (k_1 + 2 k_2 n_0 ) \Delta n
	...
	Also for laterally distanced recombination centres (\textit{e.g.} pinholes \textit{versus} well covered regions) this have been reported \cite{Montcada2017}.
%	the high mobility in the electrodes is expected to equal the quasi-Fermi levels in the two adjacent recombination centres avoiding bi-exponential decays, nevertheless reports of this phenomena have been reported \cite{Montcada2017}.
	...
	While bi-exponential decays at high background illumination are often not observed, these are quite always present at lower background illumination, as in for the example reported in \cref{fig:tpv-biexp_full}.
	...
	Additionally, when studying a decay measured at very low background light intensity, we have to remember that we're out of the small perturbation regime and this could justify non-exponential decays in various ways.
		...
		\makebox[\textwidth][c]{
			\parbox{1.1\textwidth}{
				...
				\begin{subfigure}[t]{0.5\textwidth}
					...
					\subcaption{}\label{fig:}
				\end{subfigure}
				\mycaption[]{Illumination of \SI{1e-8}{suns}, corresponding to a light bias of \SI{50}{\mV}}\label{fig:}
			}
		}
	\end{figure}
	...
	What I have observed, simulating a \acr{tpv} experiment on a homojunction device with mobile ions in the absorber layer, is that ionic profile can update to the pulse-perturbed cell condition before the extra free charges recombine, if this is very slow.
	...
	So at low background light intensities, the fast component life-time could be linked to the rearrangement time of the ionic profile.
		...
		\makebox[\textwidth][c]{
			\parbox{1.1\textwidth}{
				...
				\mycaption[Simulated \glsentryshort{tpv} without and with mobile ions.]{
					...
					In (\textbf{b}) a device with mobile ions in the absorber layer is reported, the resulting decay is a simple exponential at high light biases but a bi-exponential at lower background illuminations, so the two life-times from a bi-exponential fitting are reported.
	...
	This happens with an RC time of the circuit composed by the capacitance of the device (which can be obtained \textit{via} a \acr{dc} experiment) and the \SI{1}{\Mohm} resistance of the oscilloscope or the internal device shunt resistance \cite{Tvingstedt2017}.
...
%	This "chemical" charge (charge stored in the perovskite layer) is represented as the exponential part of \cref{eq:ce_full}, so we obtain:
	\begin{dmath}\label{eq:tpv_tau_vs_intensity}
		\tau \approx (\Phi k_\Phi n_0^{\Phi-1})^{-1} = \frac{n_|i|^{1-\Phi}}{\Phi k_\Phi} \left[\exp(\frac{q\voc}{mk_|B|T}) - 1\right]^{1-\Phi} \approx \frac{n_|i|^{1-\Phi}}{\Phi k_\Phi} \exp(-\frac{(\Phi-1)q\voc}{mk_|B|T}) = \frac{n_|i|^{1-\Phi}}{\Phi k_\Phi} \exp(-\frac{q\voc}{vk_|B|T})
	\end{dmath}
	where we used that $qV \gg k_|B|T$ for all non-dark cases in order to neglect the $-1$ addend.
	The pre-factor $n_|i|^{1-\Phi}/(\Phi k_\Phi)$ represents the thermal equilibrium life-time.
...
Similarly to the example we just considered, where RC times put a cap to the slow observable life-times, \authoryear{Kiermasch2018} underlines that also the often-neglected diode-capacitor discharge could have the same effect \cite{Tvingstedt2017,Hellen2003}.
...
Joining the information about charge life-time \textit{versus} light bias from \acr{tpv} with the information about charge density \textit{versus} light bias from \acr{ce}, we obtain the relation of the charge life-time \textit{versus} charge density.
This new relationship is more insightful than the results from bare \acr{tpv}.
	...
	\makebox[\textwidth][c]{
		\parbox{1.1\textwidth}{
			...
			\mycaption[Example of TPV-CE processed in different ways.]{
				...
				In (\textbf{c}) the $\tau$ is corrected using the fitted recombination order $\Phi$ to obtain the total carriers life-time $\tau_|pfo| = \Phi\tau$.
...
From the \cref{eq:tpv_tau_order} it is clear that for the first-order reaction case it is easy to obtain $k_1 = \tau^{-1} = \tau_|pfo|^{-1}$.
One fact of \cref{eq:tpv_tau_order} for the $\Phi=1$ case have to be underlined: the small perturbations life-time for a first-order recombination does not depend on the charge density, so it would appear as a plateau in the life-time \textit{versus} charge plot \cite{Kiermasch2018} which is not usually observed.

\paragraph{From small perturbations life-time to rate constant -- higher order reaction}
For the second and higher order recombination case, to obtain $k_x$ is non-trivial as it is related to $\tau$ \textit{via} the charge density \cite{ORegan2007} and the reaction order \cite{Shuttle2008,Du2018,Barnes2011,Barnes2011a} as shown in \cref{eq:tpv_tau_order}.
...
\begin{SCfigure}
	...
	\mycaption[Scheme of TPC experiment.]{
		A device is illuminated while short circuited through a small resistance, then a short light pulse is added and the additional flowing current is integrated to give the photo-generated charge. The constant illumination intensity is changed and the measurement is repeated.}\label{fig:tpc_scheme}
\end{SCfigure}
...
\end{SCfigure}
	...
	Then an additional illumination pulse is provided \textit{via} a nitrogen laser and the voltage across the \SI{50}{\ohm} resistor is monitored using an oscilloscope connected in parallel.
%	The signal is acquired by an oscilloscope in parallel to the \SI{50}{\ohm} resistor.
	...
	Subtracting the constant current due to the background illumination and integrating the transient over time gives the charge photo-generated by the laser pulse.
...
%	we can obtain a light bias dependent capacitance.
	Integrating this capacitance profile over voltage we can obtain the extra charge density \textit{versus} light bias profile.
...
%	This allows us to estimate the capacitance of the solar cell device at open circuit with various illumination intensities.
	...
	The amount of charge photo-generated by the laser pulse $\Delta n$ is obtained from \acr{tpc}.
...
where $C$ is a light bias $V'$ dependent capacitance, $n$ is the perturbed electron density, and $V$ is the perturbed voltage.
	...
	The capacitance can be integrated over voltage for obtaining a charge density \textit{versus} light bias relationship.
...
\paragraph{Consideration on mobility limited case}
	...
	\begin{itemize}
		...
		\item The linear factor $\Delta V$ in an exponential fit $V (t) = V_0 + \Delta V \cdot e^{-t/\tau}$ was used, but it can fail if the decay does not have a simple exponential shape (often a bi-exponential, sum of two exponentials, is observed).
		\item In cases where the \acr{tpv} decays have a bi-exponential behaviour, the sum of the two linear factors $\Delta V = \Delta V_a + \Delta V_b$ in a bi-exponential fit as in \cref{eq:tpv_biexp} could improve the previous method. One have to carefully set boundary values to the fitting parameters for avoiding a fast exponential matching just some noise.
...
\begin{equation}
\jsc = n_|i|^\Phi k_\Phi \me^{\frac{q \Phi \voc}{m k_|B| T}}
\end{equation}
...
\begin{equation}\label{eq:reconstruction_voc}
	\voc = \frac{m k_|B| T}{q \Phi} \ln( \frac{\jsc}{n_|i|^\Phi k_\Phi} )
\end{equation}
...
%\begin{equation}\tau_|pfo| = \left\{\Phi k_\Phi n_|i|^{\Phi-1} [\exp(\frac{q \voc}{m k_|B| T})-1]^{\Phi-1} \right\}^{-1}\end{equation}
