		...
		\makebox[\textwidth][c]{
			\parbox{1.1\textwidth}{
				...
				\mycaption[Simulation of a complete CE experiment: charge \textit{versus} light bias with or without mobile ions.]{
					...
					The solid lines represents the linear plus exponential fit crossing (0,0) obtained with the \cref{eq:ce_full} $n_|CE| = C_|g| V + n_|pre| \{\exp[q V / (mk_|B|T)] - 1\}$ where $C_|g|$, $n_|pre|$, and $m$ are free fitting parameters whose meaning is described in the text.}\label{fig:ce_full_dd}
			}
		}
	\end{figure}
...
\begin{SCfigure}
	\centering
	\includegraphics[width=0.6\textwidth]{tpv_scheme/tpv_scheme.pdf}
