...
\section{Conventions and General Remarks}
	...
	\subsection{Sign Convention and Parameters Definitions}
		...
		\begin{equation} \label{eq:photodiode}
			J = \jsc(\phi) - J_0\left[\exp(\frac{qV}{n_|id|k_|B|T})-1\right]
		\end{equation}
		where $J_0$ is the dark diode saturation current (the current flowing in dark when applying a reverse bias, negative sign), $q$ is the elementary charge, $k_|B|$ is the Boltzmann constant, and $T$ is the temperature.
		...
		where now $J_|ph|$ is the total photo-generated current (negative sign).
		The function is now an implicit one, requiring numerical solving even for obtaining $\jsc$.
		Just for completeness we can include the leakage current due to the shunt resistance $R_|sh|$ \cite{Tvingstedt2017}:
...
\section{V\textsubscript{OC} and J\textsubscript{SC} Dependence on Light Intensity}
	...
	\subsection{\Glsentryshort{jsc} \textit{versus} $\phi$}
	...
	\makebox[\textwidth][c]{
		\parbox{1.1\textwidth}{
			...
			\mycaption[Strategies for reducing the instrumental noise in a single \glsentryshort{ce} integration.]{
				On the left, a single \gls{ce} decay from a \gls{fto}\-/\ch{d-TiO2}\-/\ch{mp-TiO2}\-/\acr{csfamapbibr}\-/\gls{tae3}\-/Au is integrated without noise reduction in (\textbf{a}), adapting the noise profile from the dark measurement and subtracting it in (\textbf{b}), or fitting the decay and integrating the fit in (\textbf{c}).
...
%	\subsection{Interpretation of Charge Extraction}\label{interpretation_ce}
		...
		$$n_|CE| = n_|DOS| \exp(\frac{qV - E_|g|}{k_|B|T}) = n_|0,CE| \exp(\frac{qV}{k_|B|T})$$
		...
		$$n_|CE| = n_|0,CE| \exp(\frac{qV}{mk_|B|T})$$
		...
		\begin{equation}\label{eq:ce_full}
			n_|CE| = C_|g| \cdot V + u_1 \cdot \left[\exp(\frac{qV}{mk_|B|T}) - 1\right]
		\end{equation}
			...
			\makebox[\textwidth][c]{
				\parbox{1.1\textwidth}{
					...
					\mycaption[Simulation of a complete CE experiment: charge \textit{versus} light bias with or without mobile ions.]{
						...
						The solid lines represents the linear plus exponential fit crossing (0,0) obtained with the equation $n = C_|g| V + u_1 \{\exp[q V / (mk_|B|T)] - 1\}$ where $C_|g|$, $P_1$, and $m$ are free fitting parameters.}\label{fig:ce_full_dd}
				}
			}
		\end{figure}
...
\begin{figure}
	...
	\mycaption[Robust and normal fitting comparison.]{Plot of a voltage profile from a single \gls{tpv} decay.
		...
		The studied device is a \gls{fto}\-/\ch{d-TiO2}\-/\acr{csfamapbibr}\-/\gls{tae4}\-/Au with 1~sun background illumination.}\label{fig:tpv_robust}
\end{figure}
...
This does not usually affect the parameter extraction from \acr{tpv} as just the high illumination intensity points are considered, as seen in \cpageref{tpv_tau_vs_intensity}, in order to study the device close to its expected working conditions.
	...
	\begin{dmath*}
		n(V_0 + \Delta V) \approx n(V_0) + \Delta V \cdot \left.\frac{\partial n}{\partial V}\right\rvert_{V=V_0} = n(V_0) + \Delta V \cdot \left[C_|g| + \frac{u_1q}{mk_|B|T}\exp(\frac{qV_0}{mk_|B|T})\right]
%		n(V_0 + \Delta V) \approx n(V_0) + \Delta V \cdot \eval{\dv{n}{V}}_{V=V_0} = n(V_0) + \Delta V \cdot \left(C_|g| + \frac{u_1q}{mk_|B|T}\exp\left(\frac{qV_0}{mk_|B|T}\right)\right)
	\end{dmath*}
...
%		\frac{\partial (n_0 + \Delta n)}{\partial t} = g - k_1(n_0 + \Delta n) - k_2(n_0 + \Delta n)^2 = (g - k_1 n_0 - k_2 n_0^2) - (k_1 \Delta n + 2 k_2 n_0 \Delta n) - (k_2 \Delta n ^2) \approx - (k_1 + 2 k_2 n_0 ) \Delta n
	...
	\begin{equation}\label{eq:tpv_monoexp}
		n (t) = n_0 + \Delta n_0 \cdot e^{-(k_1 + 2 k_2 n_0) t} = n_0 + \Delta n_0 \cdot e^{-t / \tau_|pfo|}
	\end{equation}
	where $\tau_|pfo| = (k_1 + 2 k_2 n_0)^{-1}$ is the pseudo first order life-time.
	For a single recombination with a generic recombination reaction order $\Phi$ we can thus write \cite{Shuttle2018}:
	\begin{equation}\label{eq:tpv_tau_order}
		\tau_|pfo| \approx (\Phi k_\Phi n_0^{\Phi-1})^{-1}
	\end{equation}
	...
	This happens with an RC time of the circuit composed by the capacitance of the device (which can be obtained via a \acr{dc} experiment) and the \SI{1}{\Mohm} resistance of the oscilloscope or the internal device shunt resistance \cite{Tvingstedt2017}.
	...
	This limitation has to taken in consideration also when performing Open Circuit Voltage Decay dynamics measurement (OCVD) \cite{Tvingstedt2017} which has not been performed in this thesis. 
	...
	Considering the pseudo first order life-time dependency from charge concentration due to a single dominant recombination mechanism from \cref{eq:tpv_tau_order} and substituting the charge with the expression for the charge from \cref{eq:ce_full} we can obtain a relation between the life-time and the \gls{voc}.
...
%	This "chemical" charge (charge stored in the perovskite layer) is represented as the exponential part of \cref{eq:ce_full}, so we obtain:
	\begin{dmath}\label{eq:tpv_tau_vs_intensity}
		\tau_|pfo| \approx (\Phi k_\Phi n_0^{\Phi-1})^{-1} = u_8\left[\exp(\frac{q\voc}{mk_|B|T}) - 1\right]^{1-\Phi} \approx u_8\exp(-\frac{(\Phi-1)q\voc}{mk_|B|T}) = u_8\exp(-\frac{q\voc}{vk_|B|T})
	\end{dmath}
...
The voltage dependency of the pseudo first order life-times $\tau_|pfo|(V)$ obtained from \acr{tpv} gets expanded with the $V(n)$ relation which can be obtained inverting the fitted function of $n(V)$ from a \acr{ce} experiment for obtaining a $\tau_|pfo|(n)$ relation.

\paragraph{Which charge to take}
...
Instead, fitting data with the exponential component of \acr{ce} only, the recombination orders obtained are between 1 and 2, which are the expected values (reported in \cref{fig:tpvce-nogeom,,fig:tpvce-nogeom-total} with the same ordering of the legend, respectively $1.6$, $1.5$, $1.9$, and $1.8$).
	...
	\makebox[\textwidth][c]{
		\parbox{1.1\textwidth}{
			...
			\begin{subfigure}[t]{0.5\textwidth}
				...
				\subcaption{Exp.\ charge from \gls{ce} and corrected with $\Phi$}\label{fig:tpvce-nogeom_total}
			\end{subfigure}
			\mycaption[Example of TPV-CE processed in different ways.]{Solid lines are the power-law fit with the formula $y = k_\Phi^{-1} x^{1-\Phi}$.
...
\paragraph{From pseudo-first order rate constant to rate constant -- first-order reaction}
...
One fact of \cref{eq:tpv_tau_order} for the $\Phi=1$ case have to be underlined: the pseudo first order life-time for a first-order recombination does not depend on the charge density, so it would appear as a plateau in the life-time \textit{versus} charge plot.

\paragraph{From pseudo-first order rate constant to rate constant -- higher order reaction}
For the second and higher order recombination case, to obtain $k_x$ is non-trivial as it is related to $\tau_|pfo|$ via the charge density \cite{ORegan2007} and the reaction order \cite{Shuttle2008,Du2018} as shown in \cref{eq:tpv_tau_order}.
From this equation we can notice that correcting the pseudo first order life-time $\tau_|pfo|$ we can easily obtain a total carrier life-time $\tau$ as:
$$\tau = \Phi \tau_|pfo| = k_\Phi^{-1} n_0^{1-\Phi}$$
...
\section{Transient PhotoCurrent (\glsentryshort{tpc})}
...
\section{Differential Capacitance (DC)}
	...
	\paragraph{Concept}
	This is a meta-measurement as it just combines the data from \acr{tpv} and \acr{tpc} without requiring any additional experimental step \cite{ORegan2005,ORegan2006,Shuttle2008}, sometimes also referred to as "differential charging".
...
\paragraph{Capacitance dependence on applied voltage}
	...
	\begin{figure}%[!hbtp]%
		...
		\end{subfigure}
		\mycaption[Capacitance dependence on applied voltage.]{In (\textbf{a}) the capacitance of a commercial capacitor is reported, it was measured using \acr{ce} with applied voltage bias instead of the classical light bias used for solar cells. The capacitance is obtained as the extracted charge over the applied voltage prior to short circuiting. In (\textbf{b}) the typical capacitance \textit{versus} voltage profile of a \gls{fto}\-/\ch{d-TiO2}\-/\ch{mp-TiO2}\-/\acr{csfamapbibr}\-/\gls{tae1}\-/Au device is shown as measured by \acr{dc}. In this case the indicated voltage is originated by various illumination intensities at open circuit prior to short circuiting.}\label{fig:cap_voltage_dependence}
	}}
	\end{figure}
	...
	\begin{figure}
		...
		\mycaption[Comparison of methods for obtaining \glsentryshort{dc} experiments.]{The capacitance obtained from \gls{dc} experiment using different methods for extracting $\Delta V$ value.}\label{fig:dc_deltaV}
				}
	}
	\end{figure}
