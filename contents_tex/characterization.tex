...
%		Let's take some examples.
		...
		If mobile ions are present, this pulse induces some extent of ionic migration to a new profile \cite{Levine2018} (just consider that the ionic profile for a solar cell in dark and at \SI{1}{sun} is different, as the amount of electric field to screen is different).
...
\section{Current-Voltage Sweeps}
	...
	\mycaption[Kinks in J-V sweep due to autoscale.]{A current\hyp{}voltage sweep of an hysteretic perovskite solar cell with Keithley autoscale active.
...
%	Instead, for series resistance the transient method should not be needed, as the large employed voltage employed voltages are always larger than the built-in voltage, the depletion layers should be filled anyway and the transient method should not be required.
...
\section{V\textsubscript{OC} and J\textsubscript{SC} Dependence on Light Intensity}
	...
	\subsection{\Glsentryshort{voc} \textsl{versus} $\phi$ and the Ideality Factor $n_|id|$}\label{characterization_ideality}
	...
	\makebox[\textwidth][c]{
		\parbox{1.1\textwidth}{
			...
			\mycaption[Evolution of energy levels during CE.]{
				A $p$(\SI{200}{\nm})--$i$(\SI{500}{\nm})--$n$(\SI{200}{\nm}) device is simulated and the energy levels evolution during a \gls{ce} experiment are shown.
...
\end{figure}
	...
	\subsection{Interpretation of CE Single Decays}
		...
		\paragraph{\Acr{ce} time constant and \acr{tpv} time constant -- Corrections}\label{characterization_ce_correction}
	...
	\subsection{Interpretation of CE \textsl{versus} Light Bias With Mobile Ions}
		...
		The presence of mobile ions in perovskite materials which can accumulate at the perovskite/contacts interfaces, adds an additional capacitance $C_|ion|$, which replaces the geometric capacitance $C_|g|$ at low frequencies or long time scales, as we showed in \authoryear{Moia2019}.
		...
		A simulation with frozen ions (the ionic profile was stabilized at open circuit and frozen during the \acr{ce} experiment) was also performed but not reported.
...
\end{figure}
	...
	\subsection{Interpretation of TPV \textsl{versus} Light Bias Trend}
		...
		The pre\hyp{}factor $n_|eq|^{1-\Phi}/(\Phi k)$ represents the thermal equilibrium lifetime.
			...
			\makebox[\textwidth][c]{
				\parbox{1.1\textwidth}{
					...
					\begin{subfigure}[t]{0.5\textwidth}
						...
						\subcaption{Exp.\ charge from \gls{ce} and lifetime corrected with $\Phi$}\label{fig:tpvce-nogeom_total}
					\end{subfigure}
...
\section{Transient PhotoCurrent (\glsentryshort{tpc})}\label{characterization_tpc}
	...
	\begin{SCfigure}
		\centering
		\includegraphics[width=0.65\textwidth]{tpc_scheme/tpc_scheme.pdf}
