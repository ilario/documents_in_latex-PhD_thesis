...
In the first microseconds, most of the free charge flows through the resistor generating a voltage drop across it.
An oscilloscope in parallel to this resistance measures the transient voltage indicative of a current flowing through it.
...
This is repeated decreasing the light intensity from 1~sun down to dark (in dark no signal should be observed) and a single decay is measured for each illumination point, over a few tens of different illumination intensities.
...
The illumination and short circuiting switching equipment includes two transistors (in a home made circuit by Dr.\ Javier Pérez Hernández) connected to a pulse generator which provides a square pulse long at least as the measurement window.
...
%	The signal is acquired by an oscilloscope in parallel to the \SI{50}{\ohm} resistor.
	This allows us to measure a potential drop across the resistor and the related current via Ohm's law $J = V / R$.
