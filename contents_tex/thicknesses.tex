...
\section{Varying \glsentrytext{mapi} Thickness (Absorber Layer)}
	...
	%Both from \acr{ce} in \cref{fig:thicknesses-mapi-geometric-ce} and from \acr{dc} in \cref{fig:thicknesses-mapi-geometric-dc} we can see that the exponential regime in the charge accumulation, assigned to the chemical capacitance\index{chemical capacitance} of charges accumulating in the perovskite layer, is similar for all the thicknesses.
	...
	%Then we studied the carriers lifetime \textsl{via} \acr{tpv} and related these to the chemical charge (subtracting the charges stored in the geometric capacitance\index{geometric capacitance}) obtained either \textsl{via} \acr{ce} or \textsl{via} \acr{dc} we can confirm 
	%
	%\begin{figure}
	%	\makebox[\textwidth][c]{
	%		\parbox{1.1\textwidth}{
...
\section{Varying \glsentrytext{pcbm70} Thickness (\glsentryshort{etm} Layer)}
		...
		\makebox[\textwidth][c]{
			\parbox{1.1\textwidth}{
				...
				\mycaption[CE and DC of devices with different \glsentrytext{pcbm70} thicknesses, highlighting the varying geometric capacitance\index{geometric capacitance}.]{
					...
					The dark solid line is an exponential fitting following \cref{eq:ce_full} while the light solid line shows just its exponential part, ignoring the geometric capacitance\index{geometric capacitance}.
	...
	%Similarly to what observed for the geometric capacitance\index{geometric capacitance} 
