...
\section{Equipments}
	...
	\subsection{Equipments for Devices Electrical Characterization}
		...
		The voltage transients for photophysical characterization were registered with a Yokogawa DLM2052 oscilloscope with an internal resistance of \SI{1}{\mega\ohm}, connected to the device holder \textit{via} coaxial cable and to a computer \textit{via} an USB2 port (the transfer speed could be better if an Ethernet connection were used).
...
\section{Perovskite Solar Cells Fabrication}
		...
		\makebox[\textwidth][c]{
			\parbox{1.1\textwidth}{
				...
				\begin{subfigure}[t]{1\textwidth}
					\centering
					\includegraphics[width=0.5\textwidth]{device_picture/Perovskite_Palomares_ICIQ.jpeg}
	...
	\subsection{Top Cathode Perovskite Solar Cells}\label{methods_top}
		...
		The precursors solution (see \cpageref{precursors_mapicl}) was deposited \textit{via} spin coating (\SI{80}{\ul}, static dispensing, loading time \SI{5}{\s}) with an acceleration of \SI{1000}{\rpm\per\s}, a speed of \SI{1900}{\s} for \SI{40}{\s}.
		...
		This solution was filtered (\gls{ptfe}, \SI{0.2}{\um}) and deposited in a nitrogen-filled glovebox \textit{via} spin coating (static dispensing, \SI{80}{\ul}, loading time \SI{5}{\s}) with accelerations and speeds reported in \cref{pcbm_thickness}.
	...
	\subsection{Handling and Preservation}
		...
		The oxidation can be induced using dopants, for example FK 209 Co(III) TFSI salt\cite{Burschka2013} (this was not done in this thesis), or \textit{via} oxygen exposure in dark.
		...
		An analogous device kept in air but without illumination did not show any degradation as observable \textit{via} optical microscopy.
