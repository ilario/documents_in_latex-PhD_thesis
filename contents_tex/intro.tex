...
%		The carriers close to the band edges (in the bottom of conduction band or in the top of the valence band) are typically localised and have
		...
		\begin{SCfigure}
			\centering
			\includegraphics[width=0.7\textwidth]{DOS/DOS.pdf}
			\mycaption[Schematic representation of the density of states, Fermi-Dirac distribution, and electronic population for a crystalline intrinsic semiconductor.]{
				The green line indicates the density of states in a 3D crystalline solid.
		...
		n_|CB| = \frac{n_|eDOS|}{\exp[\frac{q(\bar\mu-E_|CB|)}{k_|B|T}]+1}
		\end{equation}
		...
		\paragraph{Doping}
		An non-doped semiconductor is intrinsic, which means that its Fermi level is in the middle of the band gap and that the electron density in the conduction band is equal to the holes density in the valence band.
		...
		This situation can happen in doped semiconductors, where the presence of, for example, positive ionic Schottky defects or oxidised species can compensate the electrons charge and result in a charge neutral material.
		...
		For a system at steady state (or where the conditions are varying slowly enough to allow the carriers to thermalise within an energy band) but out of thermodynamic equilibrium (\textsl{e.g.} a constant illumination hitting the system) the Fermi level is not defined any more and we have to use the quasi\hyp{}Fermi level instead.
