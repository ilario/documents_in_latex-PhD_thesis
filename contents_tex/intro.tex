...
If the material layer is thinner than the Debye length, the electric field will be screened just partially and no region will be completely field free.
	...
	Electrodynamics refer to all the phenomena intrinsically time-dependent, which varies with time.
		...
		\begin{figure}
			\makebox[\textwidth][c]{
				\parbox{1.1\textwidth}{
					...
					\mycaption[Representation of current and displacement current.]{In (\textbf{a}), a mixed ionic-electronic conductor material layered between two layers of electronic conductor material.
						In (\textbf{b}), the electronic charge can cross the material's interfaces.
						In (\textbf{c}), ionic charge cannot be transferred to the electrodes, still a displacement current due to the ionic migration is generated and can be measured in the amperometer.}\label{fig:displacement_current}
				}
			}
		\end{figure}
		...
		In \cpageref{displacement_current_ionic} we will use the displacement current concept for simulating the current caused in the electrodes not by a net flux of charges through a device section but by the rearrangement of charges which not necessarily leave the device, like the ionic migration in perovskite material.
...
%		The presence of mid-gap states in the doped selective contact can mediate the recombination in a fashion similar to the aforementioned Shockley-Read-Hall mechanism.
	...
	This last case, named photogeneration, is the most interesting for photovoltaics.
		...
		Additionally, depending on the polarizability AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA
...
\begin{SCfigure}
	\centering
	\includegraphics[width=0.5\textwidth]{nrel_chart/pv-efficiencies-2019-06-13.pdf}
...
%This recombination is more important in a perovskite solar cell rather than in \gls{osc} as the built-in field is shielded by the ionic accumulation and the minority carriers concentration at the perovskite\-/contacts interfaces is higher.
	...
	Such non\hyp{}radiative recombination has been predicted to have influence on the \gls{voc} of perovskite solar cells when the relative life time is smaller than \SI{10}{\us} \cite{Tress2017,Kirchartz2019}.
