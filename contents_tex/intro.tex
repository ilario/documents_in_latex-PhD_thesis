...
\section{Why Photovoltaic}
	...
	\end{SCfigure}
		...
		For a 3D crystal, the density of states grows with the square root of the energy, as represented in \cref{fig:DOS} CITATION NEEDED.
		...
		These tails of the edges are usually localised states whose mobility is low, for this reason in amorphous solids the mobility gap is wider than just the band edges separation \cite{Stenzel2005}.
%		The carriers close to the band edges (in the bottom of conduction band or in the top of the valence band) are typically localised and have
		...
		Coherently with the Fermi level, the quasi-Fermi levels are the relevant potentials for the migration of holes in the valence band and electrons in the conduction band, treated separately.
		As the system is not in thermodynamic equilibrium, macroscopic particle currents, following quasi-Fermi level gradients, can be present and this means that the energy of this level is location dependent, and can also be time dependent.
	...
	\makebox[\textwidth][c]{
		\parbox{1.1\textwidth}{
			...
			\begin{subfigure}[t]{0.51\textwidth}
				\includegraphics[scale=0.7]{homojunction/doping-pip.pdf}
				\subcaption{Homojunction before contact}\label{fig:homojunction-doping}
			\end{subfigure}
			\qquad
			\begin{subfigure}[t]{0.51\textwidth}
				\includegraphics[scale=0.7]{homojunction/poisson_and_depletion-pip.pdf}
			...
			\mycaption[Schematics of a p-i-p homojunction, with emphasis on depletion layers and representation of Poisson equation.]{
				...
				This generates space charge layers at the interfaces, where a net charge is present.
...
\end{figure}
		...
		\paragraph{Space charge layers, Debye length and electric field screening}\label{intro-space_charge}
		...
		When this involves the majority carriers (holes for a p-type material) exiting a material and leaving behind the charge from the doping species, this is named a depletion layer and is exactly the case represented in \cref{fig:homojunction-poisson_and_depletion}.
		This space charge layer generates a strong electric field at the interfaces and the materials' free charges and polarizability (permittivity) arranges so that the non-zero field zone is as thin as possible.
		The thickness of this non-zero field region is named "Debye length" or "Thomas\hyp{}Fermi screening length" and depends on the concentration of the free carriers (which in turn depends on the doping density) and on the material permittivity.
	...
	Contrarily to electrostatics, electrodynamics refer to all the phenomena intrinsically time-dependent, which varies with time.




wikipedia:  The current density for electron flow can be shown to be proportional to the gradient in the electron quasi Fermi level.
