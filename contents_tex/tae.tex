...
\section{Introduction}
	...
	%The variation of the upper contact (\textsl{i.e.}\ the \acr{htm} in bottom cathode solar cells) guarantees that the observed differences will not be due to a different perovskite structure, whose crystallization has been reported to be sensible to the underlying contact 
...
%	Considering the built-in voltage, defined as the difference in \acr{etm} (which is \ch{TiO2} in all cases) and \acr{htm} Fermi levels, we expect it to be larger for the devices with \gls{tae4}\index{TAE-4} and smallest for the \gls{tae3}\index{TAE-3} ones.
...
%	The deviations from the substrate value are in the same direction and similar magnitudes for both substrate types, with the \gls{tae3}\index{TAE-3} showing the larger deviation both on top of \ch{TiO2} or on top of perovskite.
...
%	The much larger deviations observed when using \gls{tae3}\index{TAE-3} indicate a deviation of the vacuum level for this molecule, which in turn indicates the formation of interfacial dipoles or the lack of alignment of the \gls{tae3}\index{TAE-3} \acr{homo} with perovskite valence band.
	...
	For understanding the origin of this incoherence, the electronic band gap was measured using \acr{ce} and \acr{dc}, which was in accordion with the observed \gls{voc} with exception of the inversion of \gls{tae3}\index{TAE-3} and \gls{tae4}\index{TAE-4} molecules order.
